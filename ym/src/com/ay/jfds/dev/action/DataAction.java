...
public class DataAction extends BaseAction {
	...
	public void getSelectDataByTypeName() {
		List<DataDTO> dataList = dataService.getSelectDataByTypeName(dataTypeName);
    ...
    public void getAllChildDateByParent() {
        List<Data> dataList = dataService.findAllChilden(idParam);
        String jsonObj = new JsonMapper().toJson(dataList);
        Struts2Utils.renderJson(dataList, EncodingHeaderUtil.CACHEENCODING,
                EncodingHeaderUtil.HEADERENCODING);
    }
...
//		Object cacheForTreJson = getCacheObjByKey(idParam);
		/*if (cacheForTreJson != null) {
			treeJson = (String) cacheForTreJson;
			Struts2Utils.renderJson(treeJson);
			logger.info("从缓存中得到对象");
			return;
		}*/
		DataTreeService dataTreeService = DataTreeService.getInstance();
		dataTreeService.reloadDataTree(idParam);
		treeJson = treeService.generateJsonCheckboxTree(dataTreeService,
				false);
...
/*	public void putCacheObj(String key, Object value) {
	...
	public CacheManager getCoreEhcacheManager() {
		return coreEhcacheManager;
	}

	@Autowired
	public void setCoreEhcacheManager(CacheManager coreEhcacheManager) {
		this.coreEhcacheManager = coreEhcacheManager;
	}*/
