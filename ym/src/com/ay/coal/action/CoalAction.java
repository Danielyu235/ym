...
import com.ay.coal.service.CoalService;
import com.ay.framework.core.action.BaseAction;
import com.ay.geological.pojo.Geological;
import com.opensymphony.xwork2.ActionContext;

@SuppressWarnings("all")
public class CoalAction extends BaseAction {
	...
	public void update() {
		OperateInfo operateInfo = new OperateInfo();
		boolean flag = coalService.update(coal);
		if (flag) {
	...
	public void delete() {
		...
		if (ids.contains(",")) {
			...
			flag = coalService.delete(idsParam);
		} else {
			flag = coalService.delete(ids);
		}
		if (flag) {
	...
	public void list() {
		DataStore<Coal> dataStore = new DataStore<Coal>();
		...
		Map paramMap = BeanUtil.Bean2Map(coal);
		Page resultPage = coalService.pageQuery(paramMap, pageTemp);
		List<Coal> resultList = (List<Coal>) resultPage.getCollection();
	...
	public void exp(){
		try {
			...
			List<Coal> list = coalService.getListById(ids);
			//这一步需要改成动态
			...
			//名称这一步需要改动
			response.setHeader("Content-Disposition", "attachment;filename=" + new String("采煤专业.xls".getBytes(),"iso-8859-1")) ;
	 ...
	 * @param list 数据
	 */
    private HSSFWorkbook cerateExcelFile(String[] titles, List<Coal> list){
    	HSSFWorkbook wb = new HSSFWorkbook();//创建一个excel文档
		int tLength = titles.length;
    	
    	Sheet sheet = wb.createSheet("采煤专业危险源库");//创建一个工作簿   
	...
	public void getById() {
		Struts2Utils.renderJson(coalService.getById(id),
	...
	public CoalService getCoalService() {
		return coalService;
	}

	public void setCoalService(CoalService coalService) {
		this.coalService = coalService;
	}
