package com.ay.transport.action;
...
import com.ay.transport.service.TransportService;
...
public class TransportAction extends BaseAction {
	...
	public void update() {
		OperateInfo operateInfo = new OperateInfo();
		boolean flag = transportService.update(transport);
		if (flag) {
	...
	public void delete() {
		...
		if (ids.contains(",")) {
			...
			flag = transportService.delete(idsParam);
		} else {
			flag = transportService.delete(ids);
		}
		if (flag) {
	...
	public void list() {
		DataStore<Transport> dataStore = new DataStore<Transport>();
		...
		Map paramMap = BeanUtil.Bean2Map(transport);
		Page resultPage = transportService.pageQuery(paramMap, pageTemp);
		List<Transport> resultList = (List<Transport>) resultPage.getCollection();
	...
	public void getById() {
		Struts2Utils.renderJson(transportService.getById(id),
	...
	public void exp(){
		try {
			...
			List<Transport> list = transportService.getListById(ids);
			//这一步需要改成动态
			...
			//名称这一步需要改动
			response.setHeader("Content-Disposition", "attachment;filename=" + new String("运输专业.xls".getBytes(),"iso-8859-1")) ;
	 ...
	 * @param list 数据
	 */
    private HSSFWorkbook cerateExcelFile(String[] titles, List<Transport> list){
    	HSSFWorkbook wb = new HSSFWorkbook();//创建一个excel文档
		int tLength = titles.length;
    	
    	Sheet sheet = wb.createSheet("运输专业危险源库");//创建一个工作簿   
