...
<script type="text/javascript">
	...
	$(function () {
		...
		title = month+"年 安全管理隐患专业统计";
		init(month);
    });
    function init(month){
    	$.ajax( {
				type : 'post',
				url : '${ctx}/hiddenDanger/hiddenDangerAction!getStaff',
				data : 'hiddenDanger.discoveryTime='+month,
        ...
        $('#container').highcharts({
            ...
            tooltip: {
                ...
                pointFormat: '<tr><td style="color:{series.color};font-size:2px;">{series.name}: </td>' +
                    '<td style="font-size:2px;text-align:right;"><b>{point.y} 条（隐患）</b></td></tr>',
                footerFormat: '</table>',
                shared: true,
                useHTML: true,
...
<div>
	<form action="">
		<label style="font-size: 12px;">根据专业类型按年份统计</label><input type="text" onfocus="WdatePicker({dateFmt:'yyyy'})" class="Wdate" name="hiddenDanger.discoveryTime" id="month" />
...
<script type="text/javascript">
	function change(){
		...
		init(year);
		if(year!="")
			title = year+"年  机电系统部门电耗月统计";
		if(year=="")
			title = month+"年  机电系统部门电耗月统计";
	}
</script>
</body>
</html>
