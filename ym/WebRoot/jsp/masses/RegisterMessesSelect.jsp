...
$(document).ready(
 	function (){
 		grid = new Grid('${ctx}/registerMesses/registerMessesAction!list', 'data_list');
		grid.loadGrid();
		crud = new Crud({
			grid:grid, 
			...
			afterUpdateLoadData:function(addFormObject, data){//加载update触发的事件
				...
				+'<td class="table_con" width="25%" colspan="3"><input type="radio" value="C类" checked="checked" name="type" id="type_c" onclick="hiddentType(\'C类\')"disabled="disabled"/>C类。需区队解决的隐患<br/>'
				+'<input type="radio" value="B类" name="type" id="type_b" onclick="hiddentType(\'B类\')"disabled="disabled"/>B类。需矿（厂，处）处理的隐患<br/>'
				+'<input type="radio" value="A类" name="type" id="type_a" onclick="hiddentType(\'A类\')"disabled="disabled"/>A类。需集团公司处理的隐患<br/>'
				+'</td></tr>'
				+'<tr class="appendClass"><td class="table_nemu_new" width="25%">三色单</td>'
				+'<td class="table_con" width="25%" colspan="3">'
				+'<input type="radio" value="白色" checked="checked" name="color" id="color_w" onclick="threeColor(\'白色\');" disabled="disabled"/>白色，一般隐患'
				+'<input type="radio" value="黄色" name="color" id="color_y" onclick="threeColor(\'黄色\');"disabled="disabled"/>黄色，较大隐患'
				+'<input type="radio" value="红色" name="color" id="color_r" onclick="threeColor(\'红色\');" disabled="disabled"/>红色，重大隐患'
				+'</td></tr>');
		 		...
		 		var eles = $('<fieldset class="tests">'
					...
					+'		<textarea class="easyui-validatebox" name="registerMesses.measure" style="width: 97%; height: 70px;" data-options="validType:\"maxByteLength[255]\"" disabled="disabled"></textarea>'
					+'		</td>'
					+'	</tr>'
					+'	<tr>'
					+'		<td class="table_nemu_new" width="25%">整改人职务级别</td>'
					+'		<td class="table_con" width="25%">'
					+'		<input type="text" class="easyui-validatebox" name="registerMesses.correctivePeople"data-options="validType:\"maxByteLength[100]\"" disabled="disabled"/>'
					+'		</td>'
					+'		<td class="table_nemu_new" width="25%">单位</td>'
					+'		<td class="table_con" width="25%">'
					+'		<input type="text"  name="registerMesses.correctivePeopleUnits" disabled="disabled"/>'
					+'		</td>'
					+'	</tr>'
					+'	<tr>'
					...
					+'		<td class="table_con" width="25%">'
					+'		<input type="text" class="Wdate"onfocus="WdatePicker()" name="registerMesses.deadline"disabled="disabled"/>'
					+'		</td>'
					+'	</tr>'
					+'</table>'
					+'</fieldset>');
				...
				var el = $('<fieldset class="sta">'
					+'<legend>整改情况</legend>'
					+'<table cellpadding="5px" style="font-size:12px;margin: 0 auto;" cellspacing="1" bgcolor="#aed0ea" width="95%" class="appendTables" disabled="disabled">'
					+'	<tr>'
					+'		<td class="table_nemu_new">整改结果</td>'
					+'		<td colspan="3" class="table_con">'
					+'		<textarea class="easyui-validatebox" disabled="disabled" name="registerMesses.correctionReport"style="width: 97%; height: 70px;" data-options="validType:\"maxByteLength[255]\""></textarea>'
					+'		</td>'
					+'	</tr>'
					+'	<tr>'
					+'		<td class="table_nemu_new" width="25%">完成时间</td>'
					+'		<td class="table_con" width="25%"> <input type="text" class="Wdate" name="registerMesses.completionTime"onfocus="WdatePicker()"disabled="disabled"/>'
					+'		</td>'
					+'		<td class="table_nemu_new" width="25%">跟踪落实</td>'
					+'		<td class="table_con" width="25%">'
					+' <input type="text" class="easyui-validatebox" name="registerMesses.followUp"'
					+'		data-options="validType:\"maxByteLength[100]\"" disabled="disabled"/>'
					+'		</td>'
					+'	</tr>'
					+'</table>'
...
function gridFormatter(value, rowData, rowIndex) {
	...
	url += "<a title=\"查看\" onclick='crud.update(\""+rowId+"\");' class='btn2'><img src=\"../../images/select.png\"></a>&nbsp;&nbsp;&nbsp;&nbsp;";
	</shiro:hasPermission>
	return url;
}
  function gridFormatterColor(value, rowData, rowIndex) {
      	if(value=="红色")
      		return '<div style="background-color: red;height: 20px;width:40px;border: 1px;border-style:outset;margin: 0 auto;"></div>';
   		if(value=="黄色")
   			return '<div style="background-color: yellow;height: 20px;width:40px;border: 1px;border-style:outset;margin: 0 auto;"></div>';
      	if(value == "白色")
      		return '<div style="background-color: white;height: 20px;width:40px;border: 1px;border-style:outset;margin: 0 auto;"></div>';
  	}
  	function gridFormatterName(value, rowData, rowIndex) {
  		return "群众登记";
  	}
  	function gridFormatterState(value, rowData, rowIndex) {
  		if(value =="1"){
  			return "未落实";
  		}else if(value == "2"){
  			return "已落实";
...
<!-- 表单表头 -->
	<div style="height:74%;">
	    <table id="data_list" style="display:none">
	        <thead>
	        <tr>
	        <th align="center" field="foundUnit" width="50" formatter="gridFormatterLength" >发现单位</th>
            ...
            <th align="center" field="hiddenType" width="50" formatter="gridFormatterLength" >隐患类型</th>
            <th align="center" field="threeColorForm" width="30" formatter="gridFormatterColor" >三色单</th>
            ...
            <th align="center" field="state" width="50" formatter="gridFormatterState" >整改状态</th>
            <th align="center" field="correctivePeopleName" width="50" formatter="gridFormatterLength" >整改人</th>
			...
			<legend>隐患信息</legend>
				<table id="detailTable" cellpadding="5px" style="font-size:12px;margin: 0 auto;" cellspacing="1" bgcolor="#aed0ea" width="95%" class="appendTable">
						<tr>
							...
							data-options="validType:'maxByteLength[100]'" value="新景矿" disabled="disabled"/>
							...
							<td class="table_con" width="25%">
							<input type="text" class="easyui-my97 easyui-validatebox" data-options="required:true,readOnly:true" name="registerMesses.registerTime" disabled="disabled"/>
							</td>
						</tr>
						<tr>
							...
							data-options="validType:'maxByteLength[100]'"style="width:97%;" disabled="disabled"/>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3" class="table_con">
							<textarea class="easyui-validatebox" name="registerMesses.content"style="width: 97%; height: 70px;" data-options="validType:'maxByteLength[255]'"disabled="disabled"></textarea>
							</td>
						</tr>
				</table>
				</fieldset>
