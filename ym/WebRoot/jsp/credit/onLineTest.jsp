...
<script type="text/javascript">
 ...
 $(document).ready(
 	function (){
        ...
        grid = new Grid('${ctx}/testPaperManage/testPaperManageAction!listOnline?testPaperManage.testType=2', 'data_list');
		grid.loadGrid();
		crud = new Crud({
			grid:grid, 
   ...
   function goView(){
    ...
    var self=window.open(hr,"newwindow","alwaysRaised=yes,resizable=yes,toolbar=yes, menubar=yes,scrollbars=yes,location=no, status=yes,top=0,left=0,width="+scrWidth+",height="+scrHeight);
...
function gridFormatter(value, rowData, rowIndex) {
	...
	url += "<a title=\"在线考试\" onclick='isExam(\""+rowId+"\");' class='btn2'><img src=\"${ctx}/images/onLine.png\"></a>&nbsp;&nbsp;&nbsp;&nbsp;";
	
	return url;
}
function isExam(rowId){
	 var date=new Date;
    ...
    $.ajax({
    	type: "POST",
    	url: '${ctx}/selectPaper/selectPaperAction!isExam.action?selectPaper.paperId='+rowId+'&selectPaper.username='+userid+'&selectPaper.examTime='+mydate,
