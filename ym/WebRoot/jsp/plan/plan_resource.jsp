...
$(document).ready(
 	function (){
 		 grid = new Grid('查看救援资源', 'icon-edit',
						'${ctx}/planResource/planResourceAction!list.action?planResource.id=<%=id%>','data_list');
		//加载数据
         grid.loadGrid();
 	}
  ); 
    ...
    if (rows.length == 0) {
        ...
        $.messager.confirm('警告', '是否要删除该记录?', function (r) {
            if (r) {
                ...
                $.ajax({
                    url:'${ctx}/planResource/planResourceAction!deleteByIds?ids=' + ids,
                    method:'POST',
...
function addFunction(){
	$("#data_list_resource").datagrid({
			url:"${ctx}/rescueSupplies/rescueSuppliesAction!list",
   ...
   }else
   {
        ...
        $.messager.confirm('警告', '是否要添加该记录?', function (r) {
            if (r) {
               ...
               $.ajax({
                    url:'${ctx}/planResource/planResourceAction!add.action',
                    data:{'planResource.id':'<%=id%>','planResource.resId':resId,'planResource.name':arr[0],'planResource.type':arr[1]},
                    method:'POST',
                    success:function(data) {
                    	var data = eval('(' + data + ')');
                        if (data.operateSuccess) {
                        	$.messager.alert('成功',data.operateMessage); 
                        	window.location.href='plan_resource.jsp?id=<%=id%>';
                        } else {
                        	$.messager.alert('失败',data.operateMessage); 
                        }
                    }
