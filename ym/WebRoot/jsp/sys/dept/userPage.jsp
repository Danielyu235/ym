...
.table_nemu{font-size:13px;border:0px;background:#EAF4FC;line-height:28px;padding-left:15px; width:180px;text-align:left;padding-right:10px;}
.table_con{font-size:13px;border:0px;background:#fff;line-height:28px;padding-left:15px;width:65%;}
.btn1{BORDER: #65b5e4 1px solid; PADDING-RIGHT: 6px;  PADDING-LEFT: 6px; FONT-SIZE: 12px; FILTER: progid:DXImageTransform.Microsoft.Gradient(GradientType=0, StartColorStr=#ffffff, EndColorStr=#d7ebf9); CURSOR: hand; COLOR: black; PADDING-TOP: 3px; }
#tb{line-height:32px;height:35px;padding:10px;font-size:12px;font-weight:bold;color:#2779aa;}
.search_add{ margin:5px 0;}
</style>
...
<div class="datagrid-toolbar" id = "tb" style="padding-left:0px; height:30px;">
	<%-- 
	...
	<a href="javascript:impFunction()" style="float: left;" class="l-btn l-btn-plain">
	 ...
	 <div>
        ...
        &nbsp;&nbsp;姓名:&nbsp;&nbsp;<input class="easyui-validatebox" id="searchUserName" name="user.userName" data-options="validType:'length[1,25]'">
...
<div style="height:74%;">
    <table id="data_list" style="display:none">
        ...
        <tr>
            ...
            <th field="id" width="40%" title="用户id" formatter="gridFormatter" align="center">操作</th>
        </tr>
        </thead>
    </table>
</div>
    ...
    <form  style="padding: 10px 20px 10px 20px;" id="addForm"  method="post">
	    ...
	    <table cellpadding="5px" style="font-size:12px;"  cellspacing="1"  border="0" bgcolor="#aed0ea">
	   		<tr>
				...
				<td class="table_con"><input class="easyui-validatebox" type="text" validtype="codechick" id="addAccount" name="user.account" data-options="required: true" missingMessage="请输入帐号"/></td>
			</tr>
			<tr>
				<td class="table_nemu">密 码:</td>
				<td class="table_con"><input style="width:155px" class="easyui-validatebox" type="password" id="addPassword" name="user.plainPassword" data-options="required: true, validType:'minLength[8]'" missingMessage="请输入登录密码"/></td>
			</tr>
			<tr>
				<td class="table_nemu">确认密码：</td>
				<td class="table_con"><input style="width:155px" id="rpwd" name="rpwd" type="password" class="easyui-validatebox" required validType="equals['#addPassword']" /></td>
			</tr>
			<tr>
				<td class="table_nemu">姓名：</td>
				<td class="table_con"><input class="easyui-validatebox" id="addUserName" name="user.userName" data-options="required: true,validType:'length[0,25]',invalidMessage:'输入的内容不能超过25个字符'" missingMessage="请输入用户姓名"/></td>
			</tr>
			<tr>
				<td class="table_nemu">单位：</td>
				<td class="table_con"><input class="easyui-vlidatebox" id="addDeptName"  readonly="readonly" onclick="showDeptTreeWin();"/></td>
			</tr>
			</table>
			<br/>
	        <div  align="center" >
...
<div id="updateWin" iconCls="icon-save" title="更新用户信息">
    <form  style="padding: 10px 20px 10px 20px;" id="updateForm" method="post">
		<input type="hidden" id="updateId" name="user.id"/>
        <input type="hidden" name="user.deptId" id="updateDeptId"/>
        <input type="hidden" name="user.userType" value="${param.usertype}"/>
	    <table cellpadding="5px" style="font-size:12px;"  cellspacing="1"  border="0" bgcolor="#aed0ea">
	   		<tr>
				<td class="table_nemu">登录名:</td>
				<td class="table_con"><label type="text" class="easyui-validatebox" id="updateAccount" name="user.account"></label></td>
			</tr>
			<tr>
				<td class="table_nemu">密 码:</td>
				<td class="table_con"><input style="width:155px" class="easyui-validatebox" type="password" id="updatePassword" name="user.plainPassword" data-options="required: true, validType:'minLength[8]'"/></td>
			</tr>
			<tr>
				<td class="table_nemu">姓名:</td>
				<td class="table_con"><input class="easyui-validatebox" id="updateUserName" name="user.userName"  data-options="required: true,validType:'length[0,25]',invalidMessage:'输入的内容不能超过25个字符'"/></td>
			</tr>
			<tr>
				<td class="table_nemu">状态：</td>
				<td class="table_con"><input style="width:155px" class="easyui-combobox" id="updateUserStatus" name="user.status" data-options="valueField:'val',textField:'text', editable:false, data:[{val:'1',text:'正常'},{val:'2', text:'锁定'}]"/></td>
			</tr>
			<c:if test="${param.usertype!='3'}">
				<tr>
					<td class="table_nemu">单位：</td>
					<td class="table_con"><input class="easyui-vlidatebox" id="updateDeptName" readonly onclick="showDeptTreeWin();"/></td>
				</tr>
			</c:if>
			</table>
			<br/>
...
<div id="viewWin" iconCls="icon-save" title="查看用户信息">
    <form  style="padding: 10px 20px 10px 20px;">
	    <table cellpadding="5px" style="font-size:12px;" cellspacing="1"  border="0" bgcolor="#aed0ea">
	   		<tr>
				<td class="table_nemu">登录名:</td>
				<td class="table_con"><label class="easyui-validatebox" id="viewAccount" name="user.account"></label></td>
			</tr>
			<tr>
				<td class="table_nemu">姓名:</td>
				<td class="table_con"><label class="easyui-validatebox" id="viewUserName" name="user.userName"></label></td>
			</tr>
			<tr>
				<td class="table_nemu">状态：</td>
				<td class="table_con"><label class="easyui-validatebox" id="viewUserStatus" name="user.status"></label></td>
			</tr>
			<c:if test="${param.usertype!='3'}">
				<tr>
					<td class="table_nemu">单位：${param.usertype}</td>
					<td class="table_con"><label class="easyui-validatebox" id="viewDeptName"></label></td>
				</tr>
			</c:if>
           </table>
           <br/>
	        <div align="center">
	            <a href="#" class="easyui-linkbutton" icon="icon-print" onclick="printWin();return false;">打印</a>
            	<a href="#" class="easyui-linkbutton" icon="icon-cancel" onclick="closeAllWin();return false;">取消</a>
	        </div> 
   	</form>
</div>
...
<div id="deptTreeWin" iconCls="icon-save" title="部门树">
    <div id="p" class="easyui-panel" title="部门树" fit="true" tools="#tt" border="false" plain="true">
    ...
    <div id="tt">
        ...
        <a href="#" class="icon-treeDelete" onclick="javascript:tree.expandAllNode(true);" title="展开所有"></a>
    </div>
</div>
</div>
<div id="excelWin"  iconCls="icon-save" title="上传Excel文件">
	<form name="excelForm" method="post" enctype="multipart/form-data" 
		action="${ctx}/user/userAction!imp" style="padding: 10px 20px 10px 20px;" target="excelFrame">
...
<form name="printForm" method="post" action="${ctx}/common/print.jsp" target="_blank">
