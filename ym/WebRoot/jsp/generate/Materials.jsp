...
<script type="text/javascript">
        ...
        function addFunction() {
    		...
    		$('#addForm').dialog({
    			...
    			buttons:[{
    				...
    				handler:function() {    					
    					$('#addForm').form('submit',{
    		                url:'${ctx}/materials/materialsAction!add',
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/materials/materialsAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	$.each(data,function(i,item){
                		obj["materials."+i] = item;
                    });
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/materials/materialsAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="materials.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/materials/materialsAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
...
<!-- 	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-exp', plain:true" onclick="expFunction()">导出</a> -->
	...
	&nbsp;&nbsp;物质类型:&nbsp;&nbsp;<input type="text" name="materials.type" />
		...
		<form id="addForm" method="post" style="width: 960px; height: 500px;">
			...
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" style="width: 80%;">
						...
						<tr>
							<td class="labelTd">物质类型:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="materials.type"
