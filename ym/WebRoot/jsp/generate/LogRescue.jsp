...
<script type="text/javascript">
        ...
        function addFunction() {
    		...
    		$('#addForm').dialog({
    			...
    			buttons:[{
    				...
    				handler:function() {    					
    					$('#addForm').form('submit',{
    		                url:'${ctx}/logRescue/logRescueAction!add',
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/logRescue/logRescueAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	$.each(data,function(i,item){
                		obj["logRescue."+i] = item;
                    });
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/logRescue/logRescueAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="logRescue.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/logRescue/logRescueAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
		...
		$(document).ready(function () {
		     grid = new Grid($("title").html(), 'icon-edit',
		             '${ctx}/logRescue/logRescueAction!list',
		             'data_list','#tb');
		     grid.loadGrid();
		     viewFormInit();
		});
...
<!-- 	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-exp', plain:true" onclick="expFunction()">导出</a> -->
	...
	&nbsp;&nbsp;预案Id:&nbsp;&nbsp;<input type="text" name="logRescue.pId" />
	&nbsp;&nbsp;事故Id:&nbsp;&nbsp;<input type="text" name="logRescue.aId" />
	&nbsp;&nbsp;指挥部Id:&nbsp;&nbsp;<input type="text" name="logRescue.cId" />
	&nbsp;&nbsp;响应等级:&nbsp;&nbsp;<input type="text" name="logRescue.level" />
...
<div style="height: 500px">
    <table id="data_list" loadMsg="正在努力拉取数据中..." fit="true" fitColumns="true">
        ...
        <tr>
            ...
            <th align="center" field="pId" width="150" formatter="gridFormatterLength" >预案Id</th>
            <th align="center" field="aId" width="150" formatter="gridFormatterLength" >事故Id</th>
            <th align="center" field="cId" width="150" formatter="gridFormatterLength" >指挥部Id</th>
            <th align="center" field="level" width="150" formatter="gridFormatterLength" >响应等级</th>
	...
	<div style="display: none;">
		<form id="addForm" method="post" style="width: 960px; height: 500px;">
			<input type="hidden" name="logRescue.id"/>
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" style="width: 80%;">
					
						<tr>
							<td class="labelTd">预案Id:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="logRescue.pId"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">事故Id:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="logRescue.aId"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						
						<tr>
							<td class="labelTd">指挥部Id:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="logRescue.cId"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">响应等级:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="logRescue.level"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
