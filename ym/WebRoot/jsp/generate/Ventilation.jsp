...
<script type="text/javascript">
        ...
        function addFunction() {
    		...
    		$('#addForm').dialog({
    			...
    			buttons:[{
    				...
    				handler:function() {    					
    					$('#addForm').form('submit',{
    		                url:'${ctx}/ventilation/ventilationAction!add',
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/ventilation/ventilationAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	$.each(data,function(i,item){
                		obj["ventilation."+i] = item;
                    });
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/ventilation/ventilationAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="ventilation.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/ventilation/ventilationAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
		...
		$(document).ready(function () {
		     grid = new Grid($("title").html(), 'icon-edit',
		             '${ctx}/ventilation/ventilationAction!list',
		             'data_list','#tb');
		     grid.loadGrid();
		     viewFormInit();
		});
...
<!-- 	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-exp', plain:true" onclick="expFunction()">导出</a> -->
	...
	&nbsp;&nbsp;序号:&nbsp;&nbsp;<input type="text" name="ventilation.corpKey" />
	&nbsp;&nbsp;危险源:&nbsp;&nbsp;<input type="text" name="ventilation.dangerous" />
	&nbsp;&nbsp;可能造成的伤害和损失:&nbsp;&nbsp;<input type="text" name="ventilation.loss" />
	&nbsp;&nbsp;当事人应该采取的措施:&nbsp;&nbsp;<input type="text" name="ventilation.step" />
	&nbsp;&nbsp;适用岗位:&nbsp;&nbsp;<input type="text" name="ventilation.station" />
	&nbsp;&nbsp;适用工种:&nbsp;&nbsp;<input type="text" name="ventilation.craft" />
...
<div style="height: 500px">
    <table id="data_list" loadMsg="正在努力拉取数据中..." fit="true" fitColumns="true">
        ...
        <tr>
            ...
            <th align="center" field="craft" width="150" formatter="gridFormatterLength" >适用工种</th>
			...
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" style="width: 80%;">
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.corpKey"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">危险源:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.dangerous"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.loss"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">当事人应该采取的措施:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.step"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.station"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">适用工种:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="ventilation.craft"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
