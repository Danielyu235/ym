...
<script type="text/javascript">
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/otherMaterials/otherMaterialsAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	$.each(data,function(i,item){
                		obj["otherMaterials."+i] = item;
                    });
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/otherMaterials/otherMaterialsAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="otherMaterials.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/otherMaterials/otherMaterialsAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
...
<div style="height: 500px">
    <table id="data_list" loadMsg="正在努力拉取数据中..." fit="true" fitColumns="true">
        ...
        <tr>
            ...
            <th align="center" field="type" width="150" formatter="gridFormatterLength" >类型</th>
            <th align="center" field="description" width="150" formatter="gridFormatterLength" >危险性分类及说明</th>
