...
<script type="text/javascript">
        ...
        function addFunction() {
    		...
    		$('#addForm').dialog({
    			...
    			buttons:[{
    				...
    				handler:function() {    					
    					$('#addForm').form('submit',{
    		                url:'${ctx}/healthSafe/healthSafeAction!add',
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/healthSafe/healthSafeAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	$.each(data,function(i,item){
                		obj["healthSafe."+i] = item;
                    });
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/healthSafe/healthSafeAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="healthSafe.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/healthSafe/healthSafeAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
		...
		$(document).ready(function () {
		     grid = new Grid($("title").html(), 'icon-edit',
		             '${ctx}/healthSafe/healthSafeAction!list',
		             'data_list','#tb');
		     grid.loadGrid();
		     viewFormInit();
		});
...
<!-- 	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-exp', plain:true" onclick="expFunction()">导出</a> -->
	...
	&nbsp;&nbsp;员工工号:&nbsp;&nbsp;<input type="text" name="healthSafe.employeeId" />
	&nbsp;&nbsp;体检结果:&nbsp;&nbsp;<input type="text" name="healthSafe.source" />
	&nbsp;&nbsp;填报时间:&nbsp;&nbsp;<input type="text" name="healthSafe.date" />
	&nbsp;&nbsp;职业项数量:&nbsp;&nbsp;<input type="text" name="healthSafe.number" />
	&nbsp;&nbsp;备注:&nbsp;&nbsp;<input type="text" name="healthSafe.remark" />
...
<div style="height: 500px">
	...
	<div style="display: none;">
		<form id="addForm" method="post" style="width: 960px; height: 500px;">
			<input type="hidden" name="healthSafe.id"/>
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" style="width: 80%;">
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="healthSafe.employeeId"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">体检结果:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="healthSafe.source"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="healthSafe.date"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">职业项数量:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="healthSafe.number"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						...
						<tr>
							...
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="healthSafe.remark"
							data-options="validType:'maxByteLength[100]'" />
							</td>
