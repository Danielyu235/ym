...
<script type="text/javascript">
        ...
        function addFunction() {
    		...
    		$('#addForm').dialog({
    			...
    			buttons:[{
    				...
    				handler:function() {    					
    					$('#addForm').form('submit',{
    		                url:'${ctx}/agencies/agenciesAction!add',
        ...
        function updateFunction(rowId) {
        	$.loding();
    		$.ajax({
                url:'${ctx}/agencies/agenciesAction!getById?id=' + rowId,
                method:'POST',
                success:function (data) {
                	var obj = {};
                	...
                	$('#addForm').dialog({
            			...
            			buttons:[{
            				...
            				handler:function() {
            					$('#addForm').form('submit',{
            						url:'${ctx}/agencies/agenciesAction!update',
            		                success:function(data) {
            						$('#addForm').dialog('close');
            		                	var data = eval('(' + data + ')');
            		                    if (data.operateSuccess) {
        ...
        function viewFunction(rowId) {
    		...
    		$.ajax({
                ...
                success:function (data) {
                	$.each(data,function(i,item){
                		$('#viewForm span[name="agencies.'+i+'"]').html(item);
                    });
                	$.loded();
                	$('#viewForm').dialog({
        				title:'查看窗口',
            ...
            if (rows.length == 0) {
                ...
                $.messager.confirm('警告', '是否要删除该记录?', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url:'${ctx}/agencies/agenciesAction!delete?ids=' + ids,
                            method:'POST',
                            success:function(data) {
                            	var data = eval('(' + data + ')');
                                if (data.operateSuccess) {
		...
		$(document).ready(function () {
		     grid = new Grid($("title").html(), 'icon-edit',
		             '${ctx}/agencies/agenciesAction!list',
		             'data_list','#tb');
		     grid.loadGrid();
		     viewFormInit();
		});
...
<!-- 	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-exp', plain:true" onclick="expFunction()">导出</a> -->
	...
	&nbsp;&nbsp;救援机构:&nbsp;&nbsp;<input type="text" name="agencies.aidAgencies" />
	&nbsp;&nbsp;救援措施:&nbsp;&nbsp;<input type="text" name="agencies.measures" />
	&nbsp;&nbsp;负责人:&nbsp;&nbsp;<input type="text" name="agencies.head" />
	&nbsp;&nbsp;执行时间:&nbsp;&nbsp;<input type="text" name="agencies.executeTime" />
...
<div style="height: 500px">
    <table id="data_list" loadMsg="正在努力拉取数据中..." fit="true" fitColumns="true">
        ...
        <tr>
            ...
            <th align="center" field="aidAgencies" width="150" formatter="gridFormatterLength" >救援机构</th>
            <th align="center" field="measures" width="150" formatter="gridFormatterLength" >救援措施</th>
            <th align="center" field="head" width="150" formatter="gridFormatterLength" >负责人</th>
            <th align="center" field="executeTime" width="150" formatter="gridFormatterLength" >执行时间</th>
		...
		<form id="addForm" method="post" style="width: 960px; height: 500px;">
			...
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" style="width: 80%;">
						...
						<tr>
							<td class="labelTd">救援机构:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="agencies.aidAgencies"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">救援措施:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="agencies.measures"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
						
						<tr>
							<td class="labelTd">负责人:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="agencies.head"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						
							<td class="labelTd">执行时间:</td>
							<td class="contentTd">
							<input type="text" class="easyui-validatebox" name="agencies.executeTime"
							data-options="validType:'maxByteLength[100]'" />
							</td>
						</tr>
