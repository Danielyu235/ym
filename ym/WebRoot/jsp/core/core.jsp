...
<style>
.btn1{
BORDER: #65b5e4 1px solid; PADDING-RIGHT: 6px;  PADDING-LEFT: 6px; FONT-SIZE: 12px; FILTER: progid:DXImageTransform.Microsoft.Gradient(GradientType=0, StartColorStr=#ffffff, EndColorStr=#d7ebf9); CURSOR: hand; COLOR: black; PADDING-TOP: 3px; 
}
#tb{line-height:32px;height:35px;padding:10px;font-size:12px;font-weight:bold;color:#2779aa;}
.labelTd{text-align:right; padding-right:15px;}
</style>
...
<body>	
	<div id="tb">
		...
		<c:forEach var="m" items="${menuOptList}">
			<c:if test="${m.optType=='2'}">
				...
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-${m.optCode}', plain:true" onclick="${m.optCode}Function();return false;">${m.optName}</a>
				</c:if>
				<c:if test="${empty m.funcContent && not empty m.funcName }">
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-${m.optCode}', plain:true" onclick="${m.funcName}();return false;">${m.optName}</a>
				</c:if>
				<c:if test="${not empty m.funcContent }">
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-${m.optCode}', plain:true" 
		...
		<div>
			<c:forEach var="dto" items="${listDTO}">
				<c:if test="${dto.isSearch eq '1'}">
					   <c:if test="${dto.controlType eq 'onetext' || dto.controlType eq 'telephone'}">
							      &nbsp;&nbsp;${dto.viewName}:&nbsp;&nbsp;<input type="text" id="search${dto.fieldCode}" name="${dto.fieldCode}" />
						</c:if>
						<c:if test="${dto.controlType eq 'datetimetext'}">
							&nbsp;&nbsp;${dto.viewName}:&nbsp;&nbsp;<input type="text" class="easyui-datetimebox" id="search${dto.fieldCode}" name="${dto.fieldCode}" 
												data-option="editable:false"/>
						</c:if>
						<c:if test="${dto.controlType eq 'datetext'}">
							&nbsp;&nbsp;${dto.viewName}:&nbsp;&nbsp;<input type="text" class="easyui-datebox" id="search${dto.fieldCode}" name="${dto.fieldCode}" 
							data-option="editable:false"/>
						</c:if>
						<c:if test="${dto.controlType eq 'pulltexttype' }">
								&nbsp;&nbsp;${dto.viewName}:&nbsp;&nbsp;<input id="search${dto.fieldCode}" name="${dto.fieldCode}" class="easyui-combobox" 
						...
						<c:if test="${dto.controlType eq 'numbertext'}">
							&nbsp;&nbsp;${dto.viewName}:&nbsp;&nbsp;<input id="search${dto.fieldCode}" name="${dto.fieldCode}" class="easyui-numberbox" />
						</c:if>
				</c:if>
			</c:forEach>      
			&nbsp;&nbsp;<a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="searchGrid();return false;">查询</a>
	        ...
	        <tr>
	        	<c:forEach var="dto" items="${listDTO}">
	        		<c:if test="${dto.isView eq '1'}">
	        			<th align="center" field="${dto.fieldCode}" width="${dto.width}" title="${dto.fieldName}" <c:if test="${dto.orderType eq '1'}">sortable=true</c:if>>${dto.viewName}</th>
	        		</c:if>
	        	</c:forEach>
	        	<th align="center" field="id" width="160" title="id" formatter="gridFormatter">操作</th>
	        </tr>
	        </thead>
	    </table>
	</div>
	...
	<div id="addForm" style="width:960px;height:500px;">
		<form id="saveForm" name="saveForm" method="post" action="${ctx}/core/coreAction!save.action">
		...
		<table class="detailTable" style="width: 80%;">
			...
			<c:forEach var="dto" items="${listDTO}" varStatus="status">
				...
				<c:choose>
				<c:when test="${dto.controlType eq 'numbertext' || dto.controlType eq 'onetext'
					||dto.controlType eq 'multitext'}">
					<c:if test="${not empty dto.fieldLength && dto.fieldLength != 0 && dto.controlType != 'numbertext'}">
						<c:if test="${not empty dataOptions }">
						<c:set var="dataOptions" value="${dataOptions},"></c:set>
						</c:if>
						<c:set var="dataOptions" value="${dataOptions}validType:'maxByteLength[${dto.fieldLength}]'"></c:set>
					</c:if>
					<c:if test="${dto.controlType eq 'numbertext'}">
						<c:if test="${not empty dataOptions }">
						<c:set var="dataOptions" value="${dataOptions},"></c:set>
						</c:if>
						<c:set var="dataOptions" value="${dataOptions}validType:'systemInt'"></c:set>
					</c:if>
				</c:when>
				<c:when test="${dto.controlType eq 'telephone'}">
					<c:if test="${not empty dataOptions }">
						...
						<c:set var="dataOptions" value="${dataOptions}validType:'phoneNumber'"></c:set>
				</c:when>
				<c:otherwise>
					<c:if test="${not empty dataOptions }">
						<c:set var="dataOptions" value=",${dataOptions}"></c:set>
				...
				<c:if test="${rowStatus}">
					<tr>
						...
						<c:choose>
							<c:when test="${dto.controlType eq 'onetext' || dto.controlType eq 'telephone'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-validatebox" id="add${dto.fieldCode}" name="${dto.fieldCode}" data-options="${dataOptions }" /></td>
							</c:when>
							<c:when test="${dto.controlType eq 'multitext'}">
								<c:if test="${not rowStatus}">
									<td colspan="2" class="contentTd"></td></tr><tr>							
								</c:if>
								<c:if test="${rowStatus}"><c:set var="rowStatus" value="${not rowStatus }"></c:set></c:if>
								<td class="labelTd">${dto.viewName}:</td><td colspan="3" class="contentTd"><textarea id="add${dto.fieldCode}" class="easyui-validatebox" name="${dto.fieldCode}" style="width:80%;height:80px;"  data-options="${dataOptions }" ></textarea></td>
							</c:when>
							<c:when test="${dto.controlType eq 'datetimetext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-datetimebox" id="add${dto.fieldCode}" name="${dto.fieldCode}" data-options="editable:false${dataOptions }"/></td>
							</c:when>
							<c:when test="${dto.controlType eq 'datetext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-datebox" id="add${dto.fieldCode}" name="${dto.fieldCode}" data-options="editable:false${dataOptions }"/></td>
							</c:when>
							<c:when test="${dto.controlType eq 'selectpersontext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-combotree"
							...
							<c:when test="${dto.controlType eq 'pulltexttype' }">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input id="add${dto.fieldCode}" name="${dto.fieldCode}" class="easyui-combobox" 
							...
							<c:when test="${dto.controlType eq 'onecheckboxtype'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><%=(String) request.getAttribute("${dto.fieldCode}")%></td>
							</c:when>
							<c:when test="${dto.controlType eq 'multicheckboxtype'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><%=(String) request.getAttribute("${dto.fieldCode}") %></td>
							</c:when>
							<c:when test="${dto.controlType eq 'outkey_relate'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-combobox" 
								data-options="url:'${ctx}/core/coreAction!getComboboxData.action?parentTableParam=${parentName}:${dto.relatedCode}', required:true,
								valueField:'id', textField:'value', editable:false${dataOptions}" id="add${dto.fieldCode}" name="${dto.fieldCode}" 
								panelHeight="auto"/></td>
							</c:when>
							<c:when test="${dto.controlType eq 'numbertext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-numberbox" id="add${dto.fieldCode}" name="${dto.fieldCode}"  data-options="${dataOptions }" /></td>
							</c:when>
						</c:choose>
				<c:if test="${not rowStatus}">
					</tr>
			...
			<c:if test="${rowStatus}">
				<td colspan="2" class="contentTd"></td></tr>
			</c:if>
		</table>
		</div>
		</form>
	...
	<div id="updateForm" style="width:960px;height:500px;">
		<form id="updateFormF" name="updateFormF" method="post" action="${ctx}/core/coreAction!update.action">
			...
			<c:forEach var="dto" items="${listDTO}" varStatus="status">
				...
				<c:if test="${rowStatus}">
					<tr>
						...
						<c:choose>
							<c:when test="${dto.controlType eq 'onetext' || dto.controlType eq 'telephone'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-validatebox" id="update${dto.fieldCode}" name="${dto.fieldCode}" data-options="${dataOptions }"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> /></td>
							</c:when>
							<c:when test="${dto.controlType eq 'multitext'}">
								<c:if test="${not rowStatus}">
									<td colspan="2" class="contentTd"></td></tr><tr>							
								...
								<c:if test="${rowStatus}"><c:set var="rowStatus" value="${not rowStatus }"></c:set></c:if>
								<td class="labelTd">${dto.viewName}:</td><td colspan="3" class="contentTd"><textarea class="easyui-validatebox" id="update${dto.fieldCode}" name="${dto.fieldCode}" style="width:80%;height:80px;" <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> ></textarea></td>
							</c:when>
							<c:when test="${dto.controlType eq 'datetimetext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-datetimebox" id="update${dto.fieldCode}" name="${dto.fieldCode}" data-options="editable:false${dataOptions }"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> /></td>
							</c:when>
							<c:when test="${dto.controlType eq 'datetext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-datebox" id="update${dto.fieldCode}" name="${dto.fieldCode}" data-options="editable:false${dataOptions }"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> /></td>
							</c:when>
							<c:when test="${dto.controlType eq 'selectpersontext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-combotree"
								data-options="url:'${ctx}/user/userAction!reloadComboUserTree.action',required:true,valueField:'id',textField:'text',editable:false${dataOptions}" 
								id="update${dto.fieldCode}" name="${dto.fieldCode}"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> /></td>
							</c:when>
							<c:when test="${dto.controlType eq 'selectdepttext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-combotree"
								data-options="url:'${ctx}/dept/deptAction!reloadDeptComboTree.action',required:true,valueField:'id',textField:'text',editable:false${dataOptions}" 
							...
							<c:when test="${dto.controlType eq 'pulltexttype' }">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input id="update${dto.fieldCode}" name="${dto.fieldCode}" class="easyui-combobox" 
								data-options="valueField:'id', textField:'dataName', url:'${ctx }/data/dataAction!getDataByType.action?idParam=${dto.dataTypeId}',editable:false${dataOptions}" 
							...
							<c:when test="${dto.controlType eq 'outkey_relate'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-combobox" 
								data-options="url:'${ctx}/core/coreAction!getComboboxData.action?parentTableParam=${parentName}:${dto.relatedCode}', required:true,
								valueField:'id', textField:'value', editable:false${dataOptions}" id="update${dto.fieldCode}" name="${dto.fieldCode}"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> 
								panelHeight="auto"/></td>
							</c:when>
							<c:when test="${dto.controlType eq 'numbertext'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd"><input type="text" class="easyui-numberbox" id="update${dto.fieldCode}" name="${dto.fieldCode}"  
								data-options="${dataOptions }"  <c:if test="${dto.editType eq '0' }">disabled="true"</c:if> /></td>
							</c:when>
						</c:choose>
	...
	<div id="viewForm" style="width:960px;height:500px;">
		...
		<table class="detailTable" style="width: 80%;">
			...
			<c:forEach var="dto" items="${listDTO}" varStatus="status">
				...
				<c:if test="${not empty dto.fieldLength && dto.fieldLength != 0}">
					<c:if test="${not empty dataOptions }">
					<c:set var="dataOptions" value="${dataOptions},"></c:set>
					</c:if>
					<c:set var="dataOptions" value="${dataOptions}validType:'length[0,${dto.fieldLength}]'"></c:set>
				</c:if>
				<c:if test="${rowStatus}">
					<tr>
				</c:if>
						<c:choose>
							<c:when test="${dto.controlType eq 'onetext' || dto.controlType eq 'datetimetext'
									...
									|| dto.controlType eq 'telephone'}">
								<td class="labelTd">${dto.viewName}:</td><td class="contentTd">
									<span id="view${dto.fieldCode}" name="${dto.fieldCode}" class="oneLineText"></span> 
								</td>
							</c:when>
							<c:when test="${dto.controlType eq 'multitext'}">
								<c:if test="${not rowStatus}">
								...
								<c:if test="${rowStatus}"><c:set var="rowStatus" value="${not rowStatus }"></c:set></c:if>
								<td class="labelTd">${dto.viewName}:</td><td colspan="3" class="contentTd">
								<span id="view${dto.fieldCode}" name="${dto.fieldCode}" style="width:100%;height:80px;"></span>
								</td>
							</c:when>
						</c:choose>
				<c:if test="${not rowStatus}">
				...
				<c:forEach var="childFormDTO" items="${childDTO.formDTO}">
					<c:if test="${childFormDTO.isView eq '1'}">
						...
						<c:if test="${childFormDTO.controlType != 'outkey_relate'}">
	        				<th field="${childFormDTO.fieldCode}" width="${childFormDTO.width}" title="${childFormDTO.fieldName}" <c:if test="${childFormDTO.orderType eq '1'}">sortable=true</c:if>>${childFormDTO.viewName}</th>
	        			</c:if>
	        		</c:if>
				</c:forEach>
			</tr>
	...
	<form id="expForm">
		...
		<input type="hidden" id="searchMap" name="searchMap" value="" />
		<input type="hidden" name="filterParameterNames" value="${filterParameterNames}" />
		<input type="hidden" name="filterParameterValues" value="${filterParameterValues}" />
	</form>
	</shiro:hasPermission>
	...
	<form name="excelForm" method="post" enctype="multipart/form-data" 
		action="${ctx}/core/coreAction!imp.action?menuId=${menuId}" style="padding: 10px 20px 10px 20px;" target="excelFrame">
		上传Excel文件<input type="file" name="excelFile" id="excelFile"/>
		<input type="submit" value="上传"/>
	</form>
	<iframe id="excelFrame" name="excelFrame" frameborder="0" width="100%" height="100"></iframe>
	</div>
	</div>
	</shiro:hasPermission>
	<!-- 打开方式为弹出框 -->
	<iframe id="alertFrame" name="alertFrame" frameborder="0"  style="width:100%;"></iframe>
	<script type="text/javascript">
	$(function(){
