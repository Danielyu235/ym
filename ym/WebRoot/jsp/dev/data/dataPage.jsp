...
<title>元数据属性设置</title>
    ...
    <script type="text/javascript">
        ...
        var tree = null;
        var key = null;
        var idParam = '<%=request.getParameter("idParam")%>';
     ...
     // 更新或者是保存时调用的方法
        function saveForm() {
        	...
        	{
        		var flag = window.confirm("您此次操作为新增菜单，继续请点击 确定，停止请点击取消！");
        		if(!flag){
        			return;
        		}
        	}
...
<div region="center" style="overflow: hidden" border="true" plain="true" id="center">
    <div>
        ...
        <a href="#" id="removeBtn" iconCls="icon-undo" onclick="removeDataNode()">删除</a>
    </div>

    <div>
        <form id="dataForm" method="post">
            <input type="hidden" id="id" name="data.id" />
            <input type="hidden" id="parentId" name="data.parentId" />
            <input type="hidden" id="status" value="1" />
            <input type="hidden" name="data.typeId" id="typeId"/>
            名称:<input class="easyui-validatebox" data-options="required:true" id="name" name="data.dataName"/></br>
            编码:<input type="text" validtype="codechick" id="code"  name="data.dataCode" required="true"/></br>
            排序:<input type="text" validtype="number" id="order" name="data.dataOrder" required="true"/>
            <a href="#" id="saveBtn" iconCls="icon-save" onclick="saveForm()">保存</a>
        </form>
    </div>
</div>
<div region="west" split="true" id="main_west" border="true" plain="true" style="width: 270px; padding1: 1px; overflow: hidden;">
    <div id="p" class="easyui-panel" title="元数据" fit="true" tools="#tt" border="false" plain="true">
